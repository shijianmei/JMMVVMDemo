...
#import "JMReserveEvluateCell.h"

@interface JMShopEvaluateView()<UITableViewDelegate,UITableViewDataSource,ReserveEvluateCellDelegate,SDPhotoBrowserDelegate>
{
...
@end
@implementation JMShopEvaluateView
...
{
    JMEvaluateModel *model = _dataArray[indexPath.row];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    JMReserveEvluateCell *cell = [tableView dequeueReusableCellWithIdentifier:reuseID];
    if (!cell) {
        cell = (JMReserveEvluateCell *)[[JMReserveEvluateCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:reuseID];
...
{
    NSDictionary *dataDict = [NSDictionary dictionaryWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"food.json" ofType:nil]];
...
- (void)analysisData:(NSDictionary *)dic
{
    ...
    if (productCommentList.count > 0) {
        ...
        NSArray *infoArray = [NSArray yy_modelArrayWithClass:[JMEvaluateModel class] json:productCommentList];// [JMEvaluateModel yy_model  arrayOfModelsFromDictionaries:productCommentList error:nil];
...
- (void)didSelectedPhotoView:(JMReserveEvluateCell *)cell withImgIndex:(NSInteger)index
{
    ...
    JMEvaluateModel *model = _dataArray[_selectedIndex];
