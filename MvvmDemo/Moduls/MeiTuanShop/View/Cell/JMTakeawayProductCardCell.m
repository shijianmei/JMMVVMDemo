...
@interface JMTakeawayProductCardCell()
{
    NSInteger  _count;//数据
}
@end

@implementation JMTakeawayProductCardCell
...
- (void)setupViews
{
    ...
    [_productImgView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(4.0*(ShopHome_Right_W-20)/7);
    }];
    ...
    [ _classImgView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        //lab_np@3x:新品，lab_like@2x：网友最爱，lab_red@2x：老板推荐，lab_sie@2x：招牌，
    NSMutableAttributedString *str = [AppHelper returnLineSpacingWithStr:@"            这里是标题20字以内，最多两行，这里是标题20字以内，最多两行" withLineSpacing:4 withTextAlignmentCenter:NSTextAlignmentLeft];
    CGSize size = [AppHelper sizeAttributedWithFont:kFont(14) Str:str withMaxWidth:ShopHome_Right_W-20];
    CGFloat height = 20;
    if (size.height > 20) {
        height = 40;
    }
