...
#import "JMShopMerchantView.h"
#import "UIView+Extension.h"
#import "JMShopEvaluateView.h"
#import "JMDynamicItem.h"
...
@interface JMShopScrollView ()<UIScrollViewDelegate,UIGestureRecognizerDelegate>
{
    UILabel        *_scrollLab;
...
@property (nonatomic , strong) JMShopHomePageView         *shopHomePageView;//商家列表主页,二级联动
@property (nonatomic , strong) JMShopMerchantView         *merchantView;//商家视图
@property (nonatomic , strong) JMShopEvaluateView         *shopEvaluateView;//评价视图
...
@property (nonatomic, strong) JMDynamicItem *dynamicItem;
@property (nonatomic, weak)   UIAttachmentBehavior *springBehavior;

@end

@implementation JMShopScrollView
...
#warning mark - 在这里暂时改变商家主页样式
        ...
        self.dynamicItem = [[JMDynamicItem alloc] init];
...
- (void)createTopMenuView
{
    ...
    [productBT setBackgroundImage:[AppHelper createImageWithColor:[UIColor whiteColor]] forState:UIControlStateHighlighted];
    ...
    [evaluateBT setBackgroundImage:[AppHelper createImageWithColor:[UIColor whiteColor]] forState:UIControlStateHighlighted];
    ...
    [merchantBT setBackgroundImage:[AppHelper createImageWithColor:[UIColor whiteColor]] forState:UIControlStateHighlighted];
...
- (JMShopHomePageView *)shopHomePageView
{
    if (!_shopHomePageView) {
        _shopHomePageView = [[JMShopHomePageView alloc]initWithFrame:CGRectMake(0, 0, self.width, _subScrollView.height)];
...
- (JMShopEvaluateView *)shopEvaluateView
{
    if (!_shopEvaluateView) {
        _shopEvaluateView = [[JMShopEvaluateView alloc]initWithFrame:CGRectMake(self.width, 0, self.width, _subScrollView.height) withGroupID:_groupId];
    }
    return _shopEvaluateView;
}

- (JMShopMerchantView *)merchantView
{
    if (!_merchantView) {
        _merchantView = [[JMShopMerchantView alloc]initWithFrame:CGRectMake(self.width*2, 0, self.width, _subScrollView.height)];
