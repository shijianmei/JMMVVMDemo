...
#import "JMShopHomePageView.h"
#import "UIView+Extension.h"
#import "JMHeaderFlowLayout.h"
#import "JMTakeawayProductCollectionCell.h"
#import "JMHeaderReusableView.h"
#import "JMShopProductListCell.h"
#import "JMTakeawayProductCardCell.h"

@interface JMShopHomePageView ()<UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate,UICollectionViewDelegate,UICollectionViewDataSource> {
    NSInteger _leftIndex;//左边被选中的索引值
    BOOL _isStop;//停止右边的代理方法
    JMHeaderFlowLayout *_layout;
...
@end
@implementation JMShopHomePageView
...
- (void)createView
{
    self.leftTabView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0,ShopHome_Right_W, self.height-_maxOffset_Y) style:UITableViewStyleGrouped];
    ...
    UILabel *line = [UITool lineLabWithFrame:CGRectMake(ShopHome_Left_W-0.5, 0, 0.5, self.height)];
...
{
    self.rightTabView = [[UITableView alloc]initWithFrame:CGRectMake(ShopHome_Left_W, 0,ShopHome_Right_W, self.height) style:UITableViewStylePlain];
...
{
        //创建流水布局
    _layout = [[JMHeaderFlowLayout alloc] init];
    _layout.headerReferenceSize = CGSizeMake(ShopHome_Right_W, 34);
    _layout.scrollDirection = UICollectionViewScrollDirectionVertical;
        //设置列表视图
    _collectionView = [[UICollectionView alloc]initWithFrame:CGRectMake(ShopHome_Left_W, 0,ShopHome_Right_W, self.height) collectionViewLayout:_layout];
        // 注册cell、sectionHeader、sectionFooter
    [_collectionView registerClass:[JMTakeawayProductCollectionCell class] forCellWithReuseIdentifier:@"collectCell"];
    [_collectionView registerClass:[JMHeaderReusableView class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"hederView"];
        //设置代理
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        JMHeaderReusableView *headerView = [_collectionView dequeueReusableSupplementaryViewOfKind:kind withReuseIdentifier:@"hederView" forIndexPath:indexPath];
...
{
    JMTakeawayProductCollectionCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"collectCell" forIndexPath:indexPath];
    return cell;
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView == self.leftTabView) {
        ...
        if (self.shopModuleType == ShopModuleTypeCard){
            return 110 + 4*(ShopHome_Right_W-20)/7;
        }else{
            return 100;
        }
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView == _leftTabView) {
        ...
        if (!cell) {
            ...
            UILabel *titleLab = [UITool createLabelWithFrame:CGRectMake(0,0, ShopHome_Left_W, 50) backgroundColor:[UIColor clearColor] textColor:kColor_darkBlackColor textSize:13 alignment:NSTextAlignmentCenter lines:1];
            titleLab.tag = 10;
        ...
        if (self.shopModuleType == ShopModuleTypeCard){
            ...
            JMTakeawayProductCardCell *cell = [tableView dequeueReusableCellWithIdentifier:cardCell];
            if (!cell) {
                cell = [[JMTakeawayProductCardCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cardCell];
            ...
            JMShopProductListCell *cell = [tableView dequeueReusableCellWithIdentifier:listCell];
            if (!cell) {
                cell = [[JMShopProductListCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:listCell];
                cell.selectionStyle = UITableViewCellSelectionStyleNone;
                cell.separatorInset = UIEdgeInsetsMake(0, 0, 0, 0 );
            }
                //            NewShopListModel *model = _dataArray[indexPath.row];
